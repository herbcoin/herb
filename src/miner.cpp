...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("HERBMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("HERBMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("HERBMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("herb-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("HERBMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running HERBMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadHERBMiner() exception");
    } catch (...) {
        LogPrintf("ThreadHERBMiner() exception");
    }

    LogPrintf("ThreadHERBMiner exiting\n");
}
