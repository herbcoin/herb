...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeHERBAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeHERBAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeHERBAmount = strAnonymizeHERBAmount.remove(strAnonymizeHERBAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHERBAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeHERBAmount * COIN) nMaxToAnonymize = nAnonymizeHERBAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeHERBAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeHERBAmount));
        strAnonymizeHERBAmount = strAnonymizeHERBAmount.remove(strAnonymizeHERBAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHERBAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
