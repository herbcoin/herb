    ...
    if (wtx.IsCoinStake()) {
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
				...
				if(nHeight <= 400000 && nHeight > 0) {
					nSubsidy = 130 * COIN;
					if(nSubsidy / 100 * 98 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				} else if (nHeight > 400000 && nHeight <= 800000) { // 500000 => LAST POW BLOCK
					nSubsidy = 80 * COIN;
					if(nSubsidy / 100 * 98 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				}else if (nHeight > 800000) { // 500000 => LAST POW BLOCK
					nSubsidy = 50 * COIN;
					if(nSubsidy / 100 * 98 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
