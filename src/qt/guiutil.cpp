...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a HERB address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no HERB: URI
...
{
    // Convert herb:// to herb:
    //
    //    Cannot handle this later, because herb:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open herb.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "HERB.lnk";
}
...
{
    return GetAutostartDir() / "herb.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a herb.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=HERB\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the herb app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add herb app to startup item list
