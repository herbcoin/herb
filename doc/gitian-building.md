...
*Setup instructions for a gitian build of HERB using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the HERB
...
Clone the git repositories for herb and gitian and then checkout the herb version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the herb repository
...
To build HERB (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the herb repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/herb/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/HERBCRYPTO/HERB
...
./bin/gbuild --commit herb=${COMMIT} --url herb=${URL} ../herb/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit herb=${COMMIT} --url herb=${URL} ../herb/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit herb=${COMMIT} --url herb=${URL} ../herb/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/herb-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/herb-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/herb-build.assert
```
...
[herb/gitian.sigs](https://github.com/herb-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
